...
export function AddEmployeeDialog() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // The document ID will be auto-generated by Firestore in a real scenario
    // or linked to an auth user ID. Here we use employeeId for simplicity in adding.
    // A better approach would be to create auth user first, then use their UID.
    // For now, we will create a document with a random ID if not linked to auth.
    // Let's assume an admin creates the user and the system generates an auth account later.
    const newDocRef = doc(collection(firestore, 'employees'));
    
    setDocumentNonBlocking(newDocRef, {
        id: newDocRef.id,
        authUid: null, // This would be updated when the user account is claimed
        ...values
    }, {});
